    ...
    <>
      ...
      <div className={`fixed left-0 top-0 h-screen w-64 bg-white border-r border-slate-200 z-[100] flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        ...
        <nav className="flex-1 py-4 px-3 overflow-y-auto">
          {menuItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium ${currentView === item.id ? 'bg-[#14532d] text-white' : 'text-slate-700 hover:bg-slate-50'}`}
            >
        ...
        <div className="p-4 border-t border-slate-100">
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg text-sm font-bold"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
